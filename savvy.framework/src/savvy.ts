...
module Savvy {
    ...
    var currentCard:HTMLElement = null;
    ...
    // transitions
    export var CUT:string = "savvy-cut";
    
    export var FLIP:string = "savvy-flip";
    
    export var SLIDE_UP:string = "savvy-slide-up";
    export var SLIDE_DOWN:string = "savvy-slide-down";
    export var SLIDE_LEFT:string = "savvy-slide-left";
    export var SLIDE_RIGHT:string = "savvy-slide-right";
    
    export var COVER_UP:string = "savvy-cover-up";
    export var COVER_DOWN:string = "savvy-cover-down";
    export var COVER_LEFT:string = "savvy-cover-left";
    export var COVER_RIGHT:string = "savvy-cover-right";
    
    export var UNCOVER_UP:string = "savvy-uncover-up";
    export var UNCOVER_DOWN:string = "savvy-uncover-down";
    export var UNCOVER_LEFT:string = "savvy-uncover-left";
    export var UNCOVER_RIGHT:string = "savvy-uncover-right";
    ...
    var xmlData:any = getJXONTree(readFile("app.xml", true));
	...
	function load(route:Route, preventHistory:boolean):void {
        var from:HTMLElement = currentCard;
        var to:HTMLElement = route.card.html;
        var transition:string = Savvy.CUT;
        ...
        var event:SavvyEvent = <SavvyEvent> document.createEvent("CustomEvent"); // we might fire Savvy.LOAD or Savvy.EXIT here, this var will contain the result
        event.continueTransition = ():void => {
            continueTransition.call(Savvy);
        }
        event.setTransition = (type:string):void => {
            transition = type;
        }
        
        if (from == null) {
            event.initCustomEvent(Savvy.LOAD, true, true, detail);
            document.body.dispatchEvent(event);
        } else {
            event.initCustomEvent(Savvy.EXIT, true, true, detail);
        ...
        function prepareTransition() {
            ...
            event.initCustomEvent(Savvy.READY, true, true, detail);
        ...
        function doTransition():void {
            ...
            to.style.zIndex = "1";
            if (from != null) { // may be the first transition
            ...
            switch (transition) {
                case Savvy.FLIP:
                    applyAnimation(from, "flipOutY", 500);
                    applyAnimation(to, "flipInY", 500, transitionComplete);
                    break;
                case Savvy.SLIDE_UP:
                    applyAnimation(from, "slideOutUp", 500);
                    applyAnimation(to, "slideInUp", 500, transitionComplete);
                    break;
                case Savvy.SLIDE_DOWN:
                    applyAnimation(from, "slideOutDown", 500);
                    applyAnimation(to, "slideInDown", 500, transitionComplete);
                    break;
                case Savvy.SLIDE_LEFT:
                    applyAnimation(from, "slideOutLeft", 250);
                    applyAnimation(to, "slideInRight", 250, transitionComplete);
                    break;
                case Savvy.SLIDE_RIGHT:
                    applyAnimation(from, "slideOutRight", 250);
                    applyAnimation(to, "slideInLeft", 250, transitionComplete);
                    break;
                case Savvy.COVER_UP:
                    applyAnimation(from, "static", 500);
                    applyAnimation(to, "slideInUp", 500, transitionComplete);
                    break;
                case Savvy.COVER_DOWN:
                    applyAnimation(from, "static", 500);
                    applyAnimation(to, "slideInDown", 500, transitionComplete);
                    break;
                case Savvy.COVER_LEFT:
                    applyAnimation(from, "static", 250);
                    applyAnimation(to, "slideInLeft", 250, transitionComplete);
                    break;
                case Savvy.COVER_RIGHT:
                    applyAnimation(from, "static", 250);
                    applyAnimation(to, "slideInRight", 250, transitionComplete);
                    break;
                case Savvy.UNCOVER_UP:
                    if (from) from.style.zIndex = "2";
                    setTimeout(function () {
                        from.style.zIndex = "0";
                    }, 250);
                    applyAnimation(from, "slideOutUp", 250, transitionComplete);
                    applyAnimation(to, "static", 250);
                    break;
                case Savvy.UNCOVER_DOWN:
                    if (from) from.style.zIndex = "2";
                    setTimeout(function () {
                        from.style.zIndex = "0";
                    }, 250);
                    applyAnimation(from, "slideOutDown", 250, transitionComplete);
                    applyAnimation(to, "static", 250);
                    break;
                case Savvy.UNCOVER_LEFT:
                    if (from) from.style.zIndex = "2";
                    setTimeout(function () {
                        from.style.zIndex = "0";
                    }, 250);
                    applyAnimation(from, "slideOutLeft", 250, transitionComplete);
                    applyAnimation(to, "static", 250);
                    break;
                case Savvy.UNCOVER_RIGHT:
                    if (from) from.style.zIndex = "2";
                    setTimeout(function () {
                        from.style.zIndex = "0";
                    }, 250);
                    applyAnimation(from, "slideOutRight", 250, transitionComplete);
                    applyAnimation(to, "static", 250);
                    break;
                case Savvy.CUT:
                default:
                    transitionComplete.call(Savvy);
                    break;
            }
        }
        ...
        function transitionComplete():void {
            ...
            currentCard = to;
            event.initCustomEvent(Savvy.ENTER, true, true, detail);
            route.card.html.dispatchEvent(event);
        }
	}

    function applyAnimation(element:HTMLElement, animation:string, t:number, callback?:Function):void {
        if (element == null) return; // element doesn't exist
    ...
    function createModel(cards:any):void {
        for(var i = 0, ii = cards.length; i < ii; i++) {
            ...
            htmlObject.setAttribute("data", "savvy:" + application.id + "/" + cards[i]["@id"]);
